...
export default {
  ...
  methods: {
    ...
    closeCamera () {
      ...
      bus.$emit('closeCamera', this.item)
    },
    openCamera () {
      if (!this.$store.state.room.course.courseState) {
        return this.$toast(this.lang.class.StartClassTips)
      ...
      bus.$emit('openCamera', this.item)
    },
    closeVoice () {
      if (!this.$store.state.room.course.courseState) {
        return this.$toast(this.lang.class.StartClassTips)
      }
      console.log('[s-view]', 'closeVoice', this.item)
      bus.$emit('closeVoice', this.item)
    },
    openVoice () {
      if (!this.$store.state.room.course.courseState) {
        return this.$toast(this.lang.class.StartClassTips)
      }
      console.log('[s-view]', 'openVoice', this.item)
      bus.$emit('openVoice', this.item)
    }
  }
}
</script>
