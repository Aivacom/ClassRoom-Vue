...
export default {
  ...
  computed: {
    users () {
      const users = cloneDeep(this.$store.state.room.users)
      const uid = this.$store.state.room.uid || this.$storage.get(STORAGE.KEY_UID)
      const arr = []
      const _users = omit(users, [uid])
      each(_users, (item, key) => {
        arr.push(item)
      })
      return arr
    },
  ...
  methods: {
    listeners () {
      ...
      bus.$on('closeVoice', data => {
        this.toggleVoice('close', data)
      })
      bus.$on('openVoice', data => {
        this.toggleVoice('open', data)
      })
    ...
    onMemberLeft (data) {
      ...
      each(data.uids, async item => {
        if (this.$store.state.room.users[item]) {
          await this.$store.dispatch('deleteRoomAttributesByKeys', [this.$store.state.room.users[item].uid])
          await this.$store.dispatch('getRoomAttributes')
        }
      })
    },
